...
const [newsSource, insightsSource, palladyneSource, plusOneSource, antiochSource, physicalAiFundsSource, mexicoSource, manufacturingSeriesSource] = await Promise.all([
...
const insightSlugs = new Set(
  Array.from(
    `${insightsSource}\n${palladyneSource}\n${plusOneSource}\n${antiochSource}\n${physicalAiFundsSource}\n${mexicoSource}\n${manufacturingSeriesSource}`.matchAll(
      /slug:\s*"([^"]+)"/g,
    ),
    (match) => match[1],
  ),
